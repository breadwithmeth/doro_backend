...
class Item{
    ...
    function getWithdrawal($data){
        $query = "
        SELECT orders.*, CONCAT(customers.last_name, ' ', customers.first_name) customer_name, CONCAT(workers.last_name, ' ', workers.first_name) worker_name, items.name FROM (SELECT items_orders.item_id item_id, items_orders.worker_id worker_id, items_orders.customer_id customer_id, items_orders.price price, items_orders.amount amount, items_orders.date_of_order date_of_order, 'item' type_of_order, order_id, note FROM items_orders
        UNION ALL
        ...
        WHERE `orders`.`item_id` is not null";
        ...
        if (isset($data['categories'])) {
            $categoryStr = implode(",", $data['categories']);
                $query .= " AND items.category_id in({$categoryStr})";
    ...
    function addSupplies($data, $worker_id){
        $item_id = $data['item_id'];
        $query = "INSERT INTO `supplies`(`supplier_id`, `item_id`, `worker_id`, `price_of_purchase`, `amount`) VALUES ('{$data['supplier_id']}', '{$item_id}', '{$worker_id}', '{$data['price_of_purchase']}', '{$data['amount']}' )";
        $result = $this->con->query($query);
        return json_encode($result);
    }
    ...
    function getItemHistory($data){
        $query = "SELECT orders.*, 
        CONCAT(customers.last_name, ' ', customers.first_name) customer_name, CONCAT(workers.last_name, ' ', workers.first_name) worker_name FROM (SELECT items_orders.item_id item_id, items_orders.worker_id worker_id, items_orders.customer_id customer_id, items_orders.price price, items_orders.amount amount, items_orders.date_of_order date_of_order, 'item' type_of_order, order_id, note, '-' type_of_record FROM items_orders
        UNION ALL
        SELECT recipe_ingredients_orders.item_id item_id, recipe_ingredients_orders.worker_id worker_id, recipe_ingredients_orders.customer_id customer_id, '0' price, recipe_ingredients_orders.amount amount, recipe_ingredients_orders.date_of_order date_of_order, 'recipe' type_of_order, order_id, CONCAT('Покупка рецепта #',order_id) note, '-' type_of_record FROM recipe_ingredients_orders
        UNION ALL
        SELECT service_ingredients_orders.item_id item_id, service_ingredients_orders.worker_id worker_id, service_ingredients_orders.customer_id customer_id, '0' price, service_ingredients_orders.amount amount, service_ingredients_orders.date_of_order date_of_order, 'recipe' type_of_order, order_id, CONCAT('Покупка услуги #',order_id) note, '-' type_of_record FROM service_ingredients_orders
                                                                                                                        UNION ALL
                                                                                                                        SELECT supplies.item_id, supplies.worker_id, '' customer_id, supplies.price_of_purchase, supplies.amount, supplies.date_of_supply date_of_order, 'supply' type_of_order, supplies.supplier_id order_id, 'Поступление' note, '+' type_of_record FROM supplies
